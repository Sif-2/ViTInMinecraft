...
import gzip
import math
import vision_transformer_2 as vits
...
class Handler():
    ...
    def eval(self, folder="", vis=False):  # HIER START
        self.log("STARTING EVAL")
        args = self.args
        resultdir = "eval-results/" + args.name + "/"
        pretrain = "NeededFiles/dino_deitsmall16_pretrain.pth"
        secondClustNum = 5
        layerToUse = [9]
        ...
        if os.path.isfile(pretrain):
            state_dict = T.load(pretrain, map_location="cpu")
            # remove `module.` prefix
            state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
            # remove `backbone.` prefix induced by multicrop wrapper
            state_dict = {k.replace("backbone.", ""): v for k, v in state_dict.items()}
            msg = vitscritic.load_state_dict(state_dict, strict=False)
            print(
                'Pretrained weights found at {} and loaded with msg: {}'.format(pretrain,
                                                                                msg))
        vitscritic = vitscritic.to(device)
        for x in vitscritic.parameters():
            x.requires_grad = False
        vitscritic.eval()
        with open("predictor", "rb") as fp:
            predictorlist = pickle.load(fp)
        qpredictorlist = predictorlist[0]
        kpredictorlist = predictorlist[1]
        vpredictorlist = predictorlist[2]
        afterPredictor = predictorlist[3]
        ...
        Y = np.expand_dims(np.all(np.load(evaldatapath + "Y.npy"), axis=-1),
                           axis=-1) if not args.resimages else np.zeros((len(X), 1, 64, 64))
        ...
        for bidx in range(0, len(X), batchsize):
            ...
            batchForVits = batch.clone().detach()
            batchForVits = resizer480(batchForVits)
            batchForVits = transform(batchForVits)
            if args.salience:
            ...
            if (batchsize == 1):
                preds.append(pred.detach().squeeze().unsqueeze(dim=0).cpu().numpy())
            else:
            ...
            # VISION TRANSFORMER
            if (useVits):
                kqvvaluelist = vitscritic.init_kmean_predictor(batchForVits)

                for pickedcell in range(len(pickedvidletter)):
                    vidletter = [pickedvidletter[pickedcell]]
                    vidhead = [pickedvidhead[pickedcell]]
                    vidcluster = pickedvidcluster[pickedcell]
                    for posid, i in enumerate(layerToUse):
                        ...
                        for f in vidletter:
                            ...
                            for j in vidhead:
                                ...
                                kmeanspredAfter = np.kron(kmeanspredAfter, np.ones((kronsize, kronsize)))
                                ...
                                #             beforeimg2[:, x2:(x2 + 2), y2:(y2 + 2)] = beforeimg2[:, x2:(x2 + 2),
                                #                                                         y2:(y2 + 2)] * 0.1
                                ...
                                #                     y2:(y2 + 16)] * 0.1
                                if (isFirst):
                                    vitsM.append([np.expand_dims(np.expand_dims(kmeanspred, axis=0), axis=0)])
                                    vitsSecondClusM.append(
                                        [np.expand_dims(np.expand_dims(kmeanspredAfter, axis=0), axis=0)])

                                else:
                                    vitsM[pickedcell].append(np.expand_dims(np.expand_dims(kmeanspred, axis=0), axis=0))
                                    vitsSecondClusM[pickedcell].append(
                                        np.expand_dims(np.expand_dims(kmeanspredAfter, axis=0), axis=0))
                isFirst = False

            mask = masker(batch, embeds)
            M.append(mask.detach().cpu().numpy())
        amountImages = len(imgs)
        ...
        for cellArr in range(len(vitsM)):
            vitsM[cellArr] = np.concatenate(vitsM[cellArr], axis=0)

        hardM = M > args.eval_thresh
        hardvitsM = []
        for elemLi in vitsM:
            hardvitsM.append(elemLi == 1)
        ...
        iou = self.get_iou(hardM.squeeze(), Y.squeeze())  # berechnen des IoU wertes
        iouList = []
        for iArr in hardvitsM:
            iouList.append(self.get_iou(iArr.squeeze(), Y.squeeze()))

        hardnewCellsFull = []
        newCellsIOUFull = []
        for cellID, chosenCellFull in enumerate(tqdm(vitsM)):
            chosenCell = vitsSecondClusM[cellID]
            newCells = []
            for k in range(secondClustNum):
                newCells.append(np.zeros((amountImages, 1, 64, 64)))

            for ximg in tqdm(range(amountImages)):
                for x in range(64):
                    for y in range(64):
                        if (chosenCellFull[ximg, 0, x, y].item() == 1):
                            secCluster = int(chosenCell[ximg][0, 0, x, y].item())
                            newCells[secCluster][ximg, 0, x, y] = 1

            hardnewCells = []
            for elemLi in newCells:
                hardnewCells.append(elemLi == 1)
            hardnewCellsFull.append(hardnewCells)
            newCellsIOU = []
            for iArr in hardnewCells:
                newCellsIOU.append(self.get_iou(iArr.squeeze(), Y.squeeze()))
            newCellsIOUFull.append(newCellsIOU)
        highestIOUValues = []
        highestIOUValuesSecondCluster = []
        for top in range(topNum):
            x = np.argmax(iouList)
            highestIOUValues.append((x, iouList[x]))
            iouList[x] = 0

            listNr, listNr2 = divmod(np.argmax(newCellsIOUFull), secondClustNum)
            highestIOUValuesSecondCluster.append((newCellsIOUFull[listNr][listNr2], (listNr, listNr2)))
            newCellsIOUFull[listNr][listNr2] = 0

        hardvitsM2 = []
        for top in range(topNum):
            hardvitsM2.append(hardvitsM[highestIOUValues[top][0]])
        for top in range(topNum):
            x3 = highestIOUValuesSecondCluster[top][1][0]
            y3 = highestIOUValuesSecondCluster[top][1][1]
            hardvitsM2.append(hardnewCellsFull[x3][y3])
        if True:
            hardvitsMCRF = hardvitsM2[-2].astype('float32')
            crfM = self.crf(imgs, hardvitsMCRF, Y)
            self.log("crfMshape", crfM.shape)
            hardvitsM2.append(crfM)
            valueIOU = self.get_iou(crfM.squeeze(), Y.squeeze())
            highestIOUValues.append(("test", valueIOU))

        hardvitsM = hardvitsM2
        ...
        if iou > self.ious[0] and args.visbesteval:
            ...
            colors = [np.concatenate(
                ((y & (1 - m)) + 0.5 * ((1 - y) & m), (y & m) + 0.5 * ((1 - y) & m), 0.5 * ((1 - y) & m)),
                axis=1).transpose(0, 2, 3, 1) if i in [0, 2, 3, 5, 6] else 0.1 * np.ones_like(masks[0], dtype=np.uint8)
                      for (i, m) in enumerate([m.astype(int) for m in allM])]  # Farben für untere bildreihe
            vitsColors = [np.concatenate(
                ((y & (1 - m)) + 0.5 * ((1 - y) & m), (y & m) + 0.5 * ((1 - y) & m), 0.5 * ((1 - y) & m)),
                axis=1).transpose(0, 2, 3, 1)
                          for (i, m) in enumerate([m.astype(int) for m in hardvitsM])]  # Farben für untere bildreihe
            # self.log("MASKS", [m.shape for m in masks])
            colorframes = [X] + colors
            vitsColorframes = [X] + vitsColors
            for cframe in vitsColorframes[1:]:
                colorframes.append(cframe)
            ...
            frames = F.interpolate(T.from_numpy(frames).float().permute(0, 3, 1, 2), scale_factor=scalef).permute(0, 2,
                                                                                                                  3,
                                                                                                                  1).numpy()
            ...
            for top in range(topNum):
                IOUVal = highestIOUValues[top][1]
                valStr = "iou= \n" + str(IOUVal)
                titles.append(valStr)
            for top in range(topNum):
                IOUVal = highestIOUValuesSecondCluster[top][0]
                valStr = "iou= \n" + str(IOUVal)
                titles.append(valStr)
            titlesarray = Image.fromarray(np.zeros((fosi * 4, frames.shape[2], 3), dtype=np.uint8))
            draw = ImageDraw.Draw(titlesarray)
            for i in range(len(allM) + 1 + len(hardvitsM)):
                text = titles[i]
            ...
            legendarray = Image.fromarray(np.zeros((fosi * (4 if True else 2), frames.shape[2], 3), dtype=np.uint8))
            ...
            for i, l in enumerate(legend):
                conditionallinebreak = "\n" if True and i > 1 else ""
                draw.text((fosi // 5 + i * legendspacing, fosi // 5), l + conditionallinebreak, font=font,
                          fill=legendcolors[i])
    ...
    def evalDiscriminator(self, folder=""):
        dino8 = False
        train = True
        useOtherFrames = False
        useSecondApproach = True
        if useSecondApproach:
            threshForImg = 0.9
        deslr = 0.0001
        discri_path = "discri/"
        os.makedirs(discri_path, exist_ok=True)
        save_path = discri_path + "discrilr=" + str(deslr) + "secAppr=" + str(useSecondApproach) + "threshForImg:" + str(threshForImg) +".pt"
        if dino8:
            pretrain = "NeededFiles/dino_deitsmall8_pretrain.pth"
            resizeSize = 128
            patchsize = 8
            kronsize = 4
        else:
        ...
        patchesRow = int(resizeSize / patchsize)
        args = self.args
        resultdir = "eval-results/" + args.name + "/"
        ...
        vitscritic = vits.__dict__['vit_small'](patch_size=patchsize, drop_path_rate=0.1, num_classes=0)
        if os.path.isfile(pretrain):
            state_dict = T.load(pretrain, map_location="cpu")
            # remove `module.` prefix
            state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
        ...
        if folder:
            X = np.concatenate(
                [np.array(Image.open(img_file_name)) for img_file_name in os.listdir(folder)]) / 255.0
        else:
            evaldatapath = "red-trees/"
            X = np.load(evaldatapath + "X.npy") / 255.0
            if args.resimages:
        ...
        if(useOtherFrames):
            startframe = 500
            lastframe = 10000
        X = X[startframe:lastframe:2]
        Y = Y[startframe:lastframe:2]
        qFrames = T.from_numpy(np.zeros((len(X), 1024, 4608))).float().to(self.device)   # frames, patches, pointvalue
        #kFrames = T.from_numpy(np.zeros((len(X), 1024, 4608))).float().to(self.device)
        #vFrames = T.from_numpy(np.zeros((len(X), 1024, 4608))).float().to(self.device)
        preds = []
        for bidx in tqdm(range(0, len(X), batchsize)):
            batch = T.from_numpy(X[bidx:bidx + batchsize]).permute(0, 3, 1, 2).float().to(
                self.device)
            if useSecondApproach:
                preds.append(valueCritic(batch))
            ...
            kvalues = []
            vvalues = []
            kqvvaluelist = vitscritic.init_kmean_predictor(batchForVits.to(device))
            for i in range(12):

                qvalues.append(kqvvaluelist[i]["qvalue"][:, :, 1:, :])  #remove cls
                kvalues.append(kqvvaluelist[i]["kvalue"][:, :, 1:, :])  #remove cls
                vvalues.append(kqvvaluelist[i]["vvalue"][:, :, 1:, :])  #remove cls
            del kqvvaluelist
            patchnum = qvalues[0].shape[2]
            pointsize = qvalues[0].shape[1] * qvalues[0].shape[3]
            ...
            for point in range(patchnum):
                ...
                for layer in range(12):
                    qvaluePoint = qvalues[layer][:, :, point, :].reshape(batchsize, pointsize)
                    kvaluePoint = kvalues[layer][:, :, point, :].reshape(batchsize, pointsize)
                    vvaluePoint = vvalues[layer][:, :, point, :].reshape(batchsize, pointsize)
                    if first:
                        qPoints = qvaluePoint
                        kPoints = kvaluePoint
                        vPoints = vvaluePoint
                        ...
                        qPoints = T.cat((qPoints, qvaluePoint), dim=1)
                        kPoints = T.cat((kPoints, kvaluePoint), dim=1)
                        vPoints = T.cat((vPoints, vvaluePoint), dim=1)
                qFrames[bidx:bidx + batchsize, point] = qPoints
                #kFrames[bidx:bidx + batchsize, point] = kPoints
                #vFrames[bidx:bidx + batchsize, point] = vPoints
        qFrames = qFrames
        #kFrames = kFrames
        #vFrames = vFrames


        descri = Discriminator()

        descri.to(device)
        if(not train):
            descri.load_state_dict(T.load(save_path, map_location=device))
            for p in descri.parameters():
                p.requires_grad = False
            descri.eval()
            qFrames = qFrames.reshape(len(X), 32, 32, 4608)
            maskList2 = []
            for bidx in tqdm(range(0, len(X), 1)):
                batch = qFrames[bidx:bidx + 1]
                mask = descri(batch)
                maskList2.append(mask.squeeze())

            mask = T.stack(maskList2)
            hardMask = mask > 0.5
            maskList = []
            hardMask = hardMask.cpu().numpy()
            for m in hardMask:
                m2 = np.kron(m, np.ones((2, 2)))
                maskList.append(m2)
            maskList = np.stack(maskList)
            maskList = np.array(maskList, dtype=bool)
            test = self.get_iou(Y.squeeze(), maskList)
            print("iou = " + str(test))
            if(False):
                hardvitsMCRF = hardvitsM[highestIOUValues[top][0]].astype('float32')
                crfM = self.crf(imgs, hardvitsMCRF, Y)
                self.log("crfMshape", crfM.shape)
                hardvitsM2.append(crfM)
                valueIOU = self.get_iou(crfM.squeeze(), Y.squeeze())
        ...
        if(train):
            ...
            yNew = np.zeros((len(Y), 32, 32))
            if useSecondApproach:
                preds = T.cat(preds, dim=0)
                for image in range(len(Y)):
                    if(preds[image] > threshForImg):
                        yNew[image] = np.ones((32, 32))

                


            else:
                for frame in tqdm(range(len(Y))):
                    for y in range(32):
                        y3 = y * 2
                        for x in range(32):
                            x3 = x * 2
                            ...
                            for x2 in range(2):
                                for y2 in range(2):
                                    ...
                                    if(Y[a, b, c, d]):
                                        pixelVal = 1
                            yNew[frame, x, y] = pixelVal


            opti = T.optim.Adam(descri.parameters(), lr=deslr, betas=(0.5, 0.999))
            yNew = yNew.reshape(200, 32 * 32)
            yNew = yNew.reshape(200 * 1024)
            qFrames = qFrames.reshape(200 * 1024, 4608)
            positivePoints = []
            negativePoints = []
            for idx, i in enumerate(tqdm(yNew)):
                if(i == 1):
                    positivePoints.append(qFrames[idx])
                else:
                    negativePoints.append(qFrames[idx])
            if useSecondApproach:
                ratioToAdd = len(positivePoints) / len(negativePoints)
                positivePoints = positivePoints * int(1 / ratioToAdd)
                print("ratio pos-neg:" + str(ratioToAdd))

            else:
                positivePoints = positivePoints * 6
            YLoader = T.from_numpy(np.ones((len(positivePoints)))).float().to(self.device)
            YTemp = T.from_numpy(np.zeros((len(negativePoints)))).float().to(self.device)
            YLoader = T.cat((YLoader, YTemp))
            positivePoints = T.stack(positivePoints).cpu().detach().numpy()
            negativePoints = T.stack(negativePoints).cpu().detach().numpy()
            del YTemp
            allPoints = T.from_numpy(np.concatenate((positivePoints, negativePoints), axis=0))
            descriLoader = T.utils.data.DataLoader(
                T.utils.data.TensorDataset(allPoints,
                                           YLoader),
                batch_size=100, shuffle=True)
            llog = []
            for epoch in tqdm(range(15)):
                for b_idx, (X, Y) in enumerate(descriLoader):
                    XP = X.to(device)
                    pred = descri(XP).squeeze()
                    loss = F.binary_cross_entropy(pred, Y)
                    opti.zero_grad()
                    loss.backward()
                    opti.step()
                ...
                plt.ylim(0, plt.ylim()[1])
                plt.legend()
                plt.savefig(discri_path + str(deslr) + "_loss.png")
            # descri(qFrames)

            print("saving:", save_path)
            T.save(descri.state_dict(), save_path)
            print("hi")
    ...
    def evalNew(self, folder="", vis=False):  # HIER START
        dino8 = True
        if dino8:
            pretrain = "NeededFiles/dino_deitsmall8_pretrain.pth"
            resizeSize = 128
            ...
            patchsize = 16
            kronsize = 2
        for layer in [7]: #range(12):
            layertouse = [layer]
            clusternum = 20
            os.makedirs("CountAndRatioList", exist_ok=True)
            self.log("STARTING EVAL")
            args = self.args
            resultdir = "eval-results/" + args.name + "/"
            ...
            Y = Y[100:300:2]
            #Y = Y[1100:2100]
            # predictorString = "Predictors/predictor" + str(layer)
            # with open(predictorString, "rb") as fp:
            #     predictorlist = pickle.load(fp)
            #     qpredictorlist = predictorlist[0]
            #     kpredictorlist = predictorlist[1]
            #     vpredictorlist = predictorlist[2]

            os.makedirs("NewPredictors", exist_ok=True)

            for bidx in range(0, len(X), len(X)):
                imgs.append(X[bidx:bidx + len(X)])

                batch = T.from_numpy(X[bidx:bidx + len(X)]).permute(0, 3, 1, 2).float().to(
                    self.device)
                ...
                kvalues = []
                vvalues = []
                for idx, z in enumerate(tqdm(batchForVits)):
                    kqvvaluelist = vitscritic.init_kmean_predictor(z.to(device).unsqueeze(0))
                    for i in layertouse:
                        ...
                        if idx == 0:
                            ...
                            qvalues[0] = T.cat((qvalues[0], kqvvaluelist[i]["qvalue"]), 0)
                            kvalues[0] = T.cat((kvalues[0], kqvvaluelist[i]["kvalue"]), 0)
                            vvalues[0] = T.cat((vvalues[0], kqvvaluelist[i]["vvalue"]), 0)
                    del kqvvaluelist
                print("\n Generate Kmean \n ")
                for z in layertouse:  # tqdm(range(len(qvalues))):
                    print("\n Layer " + str(z) + "\n")
                    qpredictor, kpredictor, vpredictor = vitscritic.getKMeansPredictor(qvalues[0], kvalues[0],
                                                                                       vvalues[0], clusternum)
                    ...
                    with open("NewPredictors/predictor" + str(z), "wb") as fp:
                        pickle.dump(finallist, fp)
                    print("war hier")
            predictorName = "NewPredictors/predictor" + str(layer)
            with open(predictorName, "rb") as fp:
                predictorlist = pickle.load(fp)
            qpredictorlist = predictorlist[0]
            kpredictorlist = predictorlist[1]
            vpredictorlist = predictorlist[2]
                    ...
                    for posid, i in enumerate(layertouse):
                        ...
                        for f in range(3):
                            ...
                            for j in range(6):
                                ...
                                kmeanspred = np.kron(kmeanspred, np.ones((kronsize, kronsize)))
                                if (isFirst):
                                    for xt in range(clusternum):
                                        counterList[f][j].append([0, 0])

                                for xi in range(64):
                                    for yi in range(64):
                                        if gtImage[xi, yi]:
                                            counterList[f][j][int(kmeanspred[xi, yi])][0] += 1
                                        else:
                                            counterList[f][j][int(kmeanspred[xi, yi])][1] += 1

                isFirst = False

                mask = masker(batch, embeds)
                M.append(mask.detach().cpu().numpy())
            with open("CountAndRatioList/countList" + str(layer), "wb") as fp:
                pickle.dump(counterList, fp)
                ratioList = []
            for letter in range(2):
                for head in range(6):
                    for cluster in range(clusternum):
                        if (counterList[letter][head][cluster][1] == 0):
                            ratio = counterList[letter][head][cluster][0]
                        else:
                            ratio = counterList[letter][head][cluster][0] / counterList[letter][head][cluster][1]
                        if (ratio >= 0):
                            ratioList.append((ratio, (letter, head, cluster)))

            with open("CountAndRatioList/ratioList" + str(layer), "wb") as fp:
                pickle.dump(ratioList, fp)

    def evalNewCompute(self, folder="", vis=False):  # HIER START
        useCrf = False
        pickedLayer = [7]#range(12) # [9, 10] # [0]            #
        for layer in tqdm(pickedLayer):
            for ratioThreshold in [1, 0.5, 0.2]:  # [0.1, 0.05, 0.01]: #
                layerToUse = [layer]
                self.log("STARTING EVAL")
                args = self.args
                resultdir = "eval-results/" + args.name + "/"
                pretrain = "NeededFiles/dino_deitsmall8_pretrain.pth"
                with open("CountAndRatioList/ratioList" + str(layer), "rb") as fp:
                    ratioList = pickle.load(fp)
                # (x, y , z) : x = key , y = head  , z = cluster
                useVits = True
                topNum = 2

                filteredRatioList = []
                for rat in ratioList:
                    if (rat[0] >= ratioThreshold):
                        filteredRatioList.append(rat)

                conFilteredRatioList = []
                lastLetter = None
                lastHead = None
                for rat in filteredRatioList:
                    currentLetter = rat[1][0]
                    currentHead = rat[1][1]
                    currentCluster = rat[1][2]
                    if (lastLetter == currentLetter and lastHead == currentHead):
                        conFilteredRatioList[-1][2].append(currentCluster)
                        lastHead = currentHead
                        lastLetter = currentLetter
                    else:
                        conFilteredRatioList.append([currentLetter, currentHead, [currentCluster]])
                        lastHead = currentHead
                        lastLetter = currentLetter
                ...
                vitscritic = vits.__dict__['vit_small'](patch_size=8, drop_path_rate=0.1, num_classes=0)
                if os.path.isfile(pretrain):
                    state_dict = T.load(pretrain, map_location="cpu")
                    # remove `module.` prefix
                    state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
                ...
                for x in vitscritic.parameters():
                    x.requires_grad = False
                vitscritic.eval()
                # predictorName = "Predictors/predictor" + str(layer)
                predictorName = "NewPredictors/predictor" + str(layer)
                with open(predictorName, "rb") as fp:
                    predictorlist = pickle.load(fp)
                qpredictorlist = predictorlist[0]
                ...
                for bidx in range(0, len(X), batchsize):
                    ...
                    if (useVits):
                        ...
                        for posid, i in enumerate(layerToUse):
                            for posx, pickedCells in enumerate(conFilteredRatioList):
                                f = pickedCells[0]
                                j = pickedCells[1]
                                qvalue = kqvvaluelist[i]["qvalue"]
                                predictor = qpredictorlist
                                ...
                                kmeanspred = np.kron(kmeanspred, np.ones((kronsize, kronsize)))
                                for clusterx in pickedCells[2]:
                                    kmeanspred[kmeanspred == clusterx] = -1
                                kmeanspred[kmeanspred != -1] = 0
                                kmeanspred[kmeanspred == -1] = 1
                                if (isFirst):
                                    vitsM.append([np.expand_dims(np.expand_dims(kmeanspred, axis=0), axis=0)])

                                else:
                                    vitsM[posx].append(np.expand_dims(np.expand_dims(kmeanspred, axis=0), axis=0))
                    isFirst = False
                ...
                if len(iouList) != 0:
                    ...
                    YTemp = Y.squeeze()
                    for idx, bestMask in enumerate(hardvitsM):
                        bestMask = bestMask.squeeze()
                        singleIouList.append([])
                        for maskInd in range(len(hardM)):
                            singleIouList[idx].append(self.get_iou(np.expand_dims(bestMask[maskInd], axis=0),
                                                                   np.expand_dims(YTemp[maskInd], axis=0)))
                    ...
                    if iou > self.ious[0] and args.visbesteval:
                        ...
                        colors = [np.concatenate(
                            ((y & (1 - m)) + 0.5 * ((1 - y) & m), (y & m) + 0.5 * ((1 - y) & m), 0.5 * ((1 - y) & m)),
                            axis=1).transpose(0, 2, 3, 1) if i in [0, 2, 3, 5, 6] else 0.1 * np.ones_like(masks[0],
                                                                                                          dtype=np.uint8)
                                  for (i, m) in enumerate([m.astype(int) for m in allM])]  # Farben für untere bildreihe
                        vitsColors = [np.concatenate(
                            ((y & (1 - m)) + 0.5 * ((1 - y) & m), (y & m) + 0.5 * ((1 - y) & m), 0.5 * ((1 - y) & m)),
                            axis=1).transpose(0, 2, 3, 1)
                                      for (i, m) in
                                      enumerate([m.astype(int) for m in hardvitsM])]  # Farben für untere bildreihe
                        colorframes = [X] + colors
                        vitsColorframes = [X] + vitsColors
                        for cframe in vitsColorframes[1:]:
                            colorframes.append(cframe)
                        ...
                        frames = np.concatenate([frames, colorframes], axis=1)
                        frames = (frames * 255).astype(np.uint8)
                        frames = F.interpolate(T.from_numpy(frames).float().permute(0, 3, 1, 2),
                                               scale_factor=scalef).permute(0, 2, 3, 1).numpy()
                        ...
                        for frame in range(len(frames)):
                            iouPerFrame = Image.fromarray(np.zeros((fosi * 4, frames.shape[2], 3), dtype=np.uint8))
                            draw = ImageDraw.Draw(iouPerFrame)
                            for xid, singleIou in enumerate(singleIouList):
                                xid2 = xid + 4
                                text = "iou:" + str(singleIou[frame])
                                x, y = fosi // 5 + 64 * scalef * xid2, fosi // 5
                                draw.text((x, y), text, font=font)
                            iouFrames.append(np.expand_dims(np.array(iouPerFrame), axis=0))
                        iouFrames = np.concatenate(iouFrames, axis=0)

                        legend = ["GREEN = True Positive", "RED = False Negative", "GRAY = False Positive",
                                  "BLACK = True Negative"]
                        legendarray = Image.fromarray(
                            np.zeros((fosi * (4 if True else 2), frames.shape[2], 3), dtype=np.uint8))
                        legendcolors = [(0, 255, 0), (255, 0, 0), (125, 125, 125), (255, 255, 255)]
                        legendspacing = int((frames.shape[2] - 2) / len(legend))
                        draw = ImageDraw.Draw(legendarray)
                        for i, l in enumerate(legend):
                        ...
                        frames = np.concatenate((titlesarray, frames, legendarray, iouFrames), axis=1)

                        if args.output_video:
                            args.output_video += "/"
                        vidwrite(
                            f"{args.output_video}layer{layer}thresh{ratioThreshold}iou={round(highestIOUValues[0][1], 3)}.mp4",
                            frames, framerate=10)
    ...
    def crf(self, imgs, mask, Y, skip=1):
        ...
        res = []
        params = []
        for param in [(a, b, c, d, e, i) for a in w1 for b in alpha for c in beta for d in w2 for e in gamma for i in
                      it]:
            M = mask[::skip]
            ...
            for i, img in enumerate(imgs[::skip]):
                ...
                if not i % 50:
                    ...
                    plt.imsave(resultdir + f"crf/{i}_img.png", img.squeeze())

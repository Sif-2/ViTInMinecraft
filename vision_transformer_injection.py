...
class VisionTransformer(nn.Module):
    ...
    def init_kmean_predictor(self, x):
        x = self.prepare_tokens(x)
        kqvlist= []
        for blk in self.blocks:
            x, kqvvalues = blk(x, savekqv=True)
            kqvlist.append(kqvvalues)
    ...
    def getKMeansPredictor(self, qvalues, kvalues, vvalues, numCluster):
        ...
        kmeansk = []
        kmeansv = []
        for h in tqdm(range(nh)):
            qtemp = qvalues[:, :, 1:, :]  # remove cls
            qtemp = qtemp[:, h, :, :]  # take head
            a = qtemp.shape[0]
            b = qtemp.shape[1]
            c = qtemp.shape[2]
            ...
            ktemp = kvalues[:, :, 1:, :]  # remove cls
            ktemp = ktemp[:, h, :, :]  # take head
            a = ktemp.shape[0]
            b = ktemp.shape[1]
            c = ktemp.shape[2]
            ...
            vtemp = vvalues[:, :, 1:, :]  # remove cls
            vtemp = vtemp[:, h, :, :]  # take head
            a = vtemp.shape[0]
            b = vtemp.shape[1]
            c = vtemp.shape[2]
